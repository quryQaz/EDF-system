...
import {InfoLogs} from "GUI/Components/Dashboard/Pages/Components/InfoLogs"
...
const Logs = (observer(class extends React.Component {
    ...
    loadData = () => {
        basicRequestPost(`${config.BACKEND_URL}/api/logs`, {ip: this.props.ip}).then((response) => {
    ...
    render() {
        return(
            <MainWrapper>
                <Header label={"Logs"}/>
                <InfoLogs items={this.items} MLActive={this.props.MLActive}/>
                <Footer/>
            </MainWrapper>
        )
    }
